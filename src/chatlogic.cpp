...
ChatLogic::ChatLogic()
{
    ...
    //*_chatBot = ChatBot("../images/chatbot.png");    //Use class constructor instead of new
    //Move instance creation of ChatBot object to LoadAnswerfromGraph method
...
ChatLogic::~ChatLogic()
{
    ...
    /*
    delete _chatBot;
    _chatBot = nullptr; //Attempt to resolve program crash on exit (task 0)
   ...
   // Deconstructor no longer needed since ChatLogic uses unique pointers (Task 3)
...
void ChatLogic::LoadAnswerGraphFromFile(std::string filename)
{
            ...
            if (type != tokens.end())
            {
                ...
                if (idToken != tokens.end())
                {
                    ...
                    if (type->second == "NODE")
                    {
                        ...
                        // Update nodes to use unique pointer in class (Task 3)
                        ...
                        if (newNode == _nodes.end())
                        {
                            ...
                            _nodes.emplace_back(std::move(std::make_unique<GraphNode>(id)));    //Update for unique pointers to GraphNode objects (Task 3)
    ...
    // identify root node
    GraphNode *rootNode = nullptr;  //Update nodes to use unique pointer in class (Task 3)
    for (auto it = std::begin(_nodes); it != std::end(_nodes); ++it)
    {
        ...
        if ((*it)->GetNumberOfParents() == 0)
        {
            ...
            if (rootNode == nullptr)
            {
                ...
                //rootNode = std::make_unique<GraphNode> it; //Update to ensure compatible unique pointer types in assignment (Task 3)
                rootNode = it->get(); //Assign current node to root node
            }
            else
            {
    ...
    //Initialize a ChatBot object on the stack (Task 5)
    ChatBot chatbot("../images/chatbot.png");
    // add chatbot to graph root node
    chatbot.SetRootNode(rootNode);
    SetChatbotHandle(&chatbot);
    chatbot.SetChatLogicHandle(this);
    //Maintain chatbot conection for GUI interface
    _chatBot = &chatbot;
